...
import purchaseAdd from './WarrantyAdd.vue'
import purchaseView from './WarrantyView.vue'
...
export default {
    ...
    columns () {
      return [{
        ...
        customRender: (text, row, index) => {
          if (text > -30 && text < 0) {
            return <a-tag color="#108ee9">近一个月内过期</a-tag>
          } else if (text > 0) {
            return <a-tag color="#f50">已过期</a-tag>
          }
        }
      }, {
        title: '药品名称',
  ...
  methods: {
    ...
    batchDelete () {
      ...
      this.$confirm({
        ...
        onOk () {
          ...
          that.$delete('/cos/warranty-info/' + ids).then(() => {
    ...
    fetch (params = {}) {
      ...
      this.$get('/cos/warranty-info/page', {
        ...params
      }).then((r) => {
