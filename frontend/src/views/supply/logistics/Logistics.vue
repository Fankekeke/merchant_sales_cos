...
<!--        <a-button type="primary" ghost @click="add">药品采购</a-button>-->
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="operation" slot-scope="text, record">
          ...
          <a-icon v-if="record.status == 1" type="stop" theme="twoTone" twoToneColor="#4a9ff5" @click="returnGoods(record)" title="退货" style="margin-left: 15px"></a-icon>
        </template>
      </a-table>
    </div>
    <purchase-view
...
export default {
    ...
    columns () {
      return [{
        ...
        customRender: (text, row, index) => {
          switch (text) {
            case -1:
              return <a-tag color="red">已退货</a-tag>
            case 1:
              return <a-tag>运输中</a-tag>
            case 2:
              return <a-tag color="green">已验收</a-tag>
            default:
              return '- -'
          }
        }
  ...
  methods: {
    ...
    returnGoods (record) {
      ...
      this.$confirm({
        title: '确定是否要退货?',
        content: '当您点击确定按钮后，这些记录将会被退货',
        centered: true,
        onOk () {
          that.$get(`/cos/purchase-info/returnGoods/${record.id}`).then((r) => {
